...
public class ArtistRepositoryTest {
    ...
    public void testFindArtistByName() {
        /*ArtistJPA artist1 = new ArtistJPA("Oliver Abc");
        ArtistJPA artist2 = new ArtistJPA("Oliver Def");
        ArtistJPA artist3 = new ArtistJPA("Oliver Jkl");

        artistRepository.create(artist1);
        artistRepository.create(artist2);
        artistRepository.create(artist3);*/

        List<ArtistJPA> artists = artistRepository.findByName("Oliver");
        assertEquals(3, artists.size());
        for(ArtistJPA artist : artists) {
            assertEquals("Oliver", artist.getName().substring(0, 6));
        }
    }
}
